...
// Use reduce to sum up the pledged amounts from all the games
...
// Use reduce to sum up the number of games
...
const displayStr = 
  `A total of $${totalPledged.toLocaleString()} has been raised for ${GAMES_JSON.length} games. ` +
  `Currently, ${numUnfundedGames} game${numUnfundedGames === 1 ? '' : 's'} ` +
  `remain${numUnfundedGames === 1 ? 's' : ''} unfunded. We need your help to fund these amazing games!`;



// create a new DOM element containing the template string and append it to the description container
document.addEventListener('DOMContentLoaded', () => {
    // Check if the element was found (if not, we can log an error)
    if (descriptionContainer) {
        ...
        console.error("descriptionContainer element not found!");
    }
});
...
// Destructuring to grab the first and second games
